...
def electricity_data():
    try:
        ...
        if month and year:
            # Construct regex pattern to match the specified month-year format
            month_abbr = month[:3].capitalize()
            year_pattern = year[-2:]  # Extract the last two digits of the year
            regex_pattern = rf'{month_abbr}-{year_pattern}$'
            query['Time'] = {'$regex': regex_pattern, '$options': 'i'}
        elif month:
            # Construct regex pattern to match any month in the specified year
            month_abbr = month[:3].capitalize()
            regex_pattern = rf'{month_abbr}-\d{{2}}$'
            query['Time'] = {'$regex': regex_pattern, '$options': 'i'}
        elif year:
            # Construct regex pattern to match any year
            year_pattern = year[-2:]  # Extract the last two digits of the year
            regex_pattern = rf'-{year_pattern}$'
            query['Time'] = {'$regex': regex_pattern, '$options': 'i'}
